...
public class AssetBundleMaker  {
    ...
    [MenuItem("Tool/更新了代码和ab资源的话，请点击这个再运行")]
    public static void ReflashAssetBundle()
    {
        MakeAssetBundle_ToStreamingAssetPath();
        AddExtension_ab_ToStreamingAssets();
    }


    //"Tool/a.生成当前平台对应的AssetBundle，放在StreamingAssetPath路径下(将删除上次生成的ab包)"
    private static void MakeAssetBundle_ToStreamingAssetPath()
    {
        ClearTaretDirAllFiles(Application.streamingAssetsPath);
        ...
        AssetDatabase.Refresh();
        //Debug.Log("a.制作新的AB包成功，位于:" + Application.streamingAssetsPath);
        string Path =   "Assets/" + "StreamingAssets"+ "/" + "scenes.ab";
        string[] files = Directory.GetFiles(Application.dataPath + "/" + Tools.AssetBundleScenesDirName, "*.unity", SearchOption.AllDirectories);
        string[] relativeFiles = new string[files.Length];
        for (int i = 0; i < files.Length; i++)
        {
    ...
    private static void AddExtension_ab_ToStreamingAssets()
    {
        string sourceFileName = Application.streamingAssetsPath + "/" + "StreamingAssets";
        string destFileName = sourceFileName + ".ab";
        File.Move(sourceFileName, destFileName);
    }
